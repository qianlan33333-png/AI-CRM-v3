...
func (r *Repository) RunAttempt(ctx context.Context, id, generation, riverJobID int64, adapter ProviderAdapter) error {
	...
	if next == StateExecuted && (envelope.Kind == KindWeComTagCatalog || envelope.Kind == KindChannelAsset) && (r.sink == nil || !adapterResult.Artifact.Valid()) {
	...
	shouldComplete := r.sink != nil && terminal && (envelope.Kind == KindGroupMessage || envelope.Kind == KindWeComTagCatalog || envelope.Kind == KindChannelAsset || envelope.Owner == OwnerPayment)
	if shouldComplete {
