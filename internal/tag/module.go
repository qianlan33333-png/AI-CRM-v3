...
func (m *ModuleRegistration) Readiness(ctx context.Context, pool *pgxpool.Pool) error {
	...
	var ready bool
	e := pool.QueryRow(ctx, `SELECT NOT EXISTS (SELECT 1 FROM unnest(ARRAY['tag_groups','tag_catalog_tags','tag_references','tag_operation_receipts','tag_audit_events','tag_outbox','tag_sync_receipts','tag_provider_observations']) AS required(name) WHERE to_regclass(current_schema() || '.' || required.name) IS NULL)`).Scan(&ready)
	if e != nil {
		return e
	}
	if !ready {
