...
func (store *PostgreSQLStore) AudienceEntries(ctx context.Context, reference time.Time) ([]channelport.AudienceEntry, error) {
	...
	rows, err := tx.Query(ctx, `WITH history_latest AS (
		...
		SELECT h.customer_id,h.channel_id,c.code AS channel_code,h.owner_reference,h.first_entered_at,h.last_entered_at FROM history_latest h JOIN channels c ON c.id=h.channel_id WHERE h.customer_id IS NOT NULL
		UNION ALL
		SELECT e.customer_id,b.channel_id,c.code AS channel_code,COALESCE(a.staff_id::text,''),e.occurred_at,e.occurred_at
