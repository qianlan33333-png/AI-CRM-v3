...
func TestMachineTokenRotateAndDisableImmediatelyInvalidateBearer(t *testing.T) {
	...
	created, err := service.Create(context.Background(), admin, CreateMachineClientInput{
		ClientID: "partner.analytics", DisplayName: "Partner analytics", Purpose: "api",
	...
	if _, err = service.AuthenticateBearer(context.Background(), issued.AccessToken, "external_integration", netip.MustParseAddr("203.0.113.9")); !errors.Is(err, domain.ErrMachineCredential) {
...
func TestMachineClientCredentialsEnforcesAudienceScopeAndCIDR(t *testing.T) {
	...
	admin := domain.Principal{Kind: domain.KindAdmin, InternalID: 9, Roles: []domain.Role{domain.RoleSuperAdmin}}
	created, err := service.Create(context.Background(), admin, CreateMachineClientInput{ClientID: "mcp.agent", DisplayName: "MCP agent", Purpose: "mcp", Audiences: []string{"mcp"}, Scopes: []string{"mcp"}, Capabilities: []string{"mcp_read", "mcp_execute"}, AllowedCIDRs: []string{"203.0.113.0/24"}, TokenTTLSeconds: 60})
	if err != nil {
		t.Fatal(err)
	}
	input := ClientCredentialsInput{ClientID: created.Client.ClientID, ClientSecret: created.Secret, Audience: "mcp", RequestedScopes: []string{"mcp"}, SourceIP: netip.MustParseAddr("203.0.113.11")}
	...
	input.SourceIP, input.Audience = netip.MustParseAddr("203.0.113.11"), "external_integration"
