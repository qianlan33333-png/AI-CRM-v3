...
dom.window.fetch = async (url, options = {}) => {
  ...
  if (target.includes("/external-push/test")) {
    testPosts += 1;
    return { ok: true, status: 202, json: async () => ({ questionnaire_id: 7, test_run_id: "questionnaire-test-0123456789abcdef0123456789abcdef", effect_id: "eer_7", status: "queued" }) };
  }
  if (target.endsWith("/external-push")) {
    const body = JSON.parse(options.body);
    return { ok: true, status: 200, json: async () => ({ configuration_version: ++metadataVersion, external_push: { enabled: body.enabled, configuration_reference: body.configuration_reference, metadata: body.metadata } }) };
  }
  if (target.includes("external-push-logs")) return { ok: true, status: 200, json: async () => ({ items: [{ source_pk: "questionnaire-test-executed", status: "executed", attempt_count: 1, provider_result_received: true, occurred_at: "2026-09-05T00:00:00Z" }, { source_pk: "questionnaire-test-unknown", status: "outcome_unknown", attempt_count: 1, occurred_at: "2026-09-05T00:00:01Z" }] }) };
  return { ok: true, status: 200, json: async () => ({ items: [{ source_pk: "questionnaire-test-queued", status: "queued", occurred_at: "2026-09-05T00:00:00Z" }], configuration_version: metadataVersion, external_push: { enabled: true, configuration_reference: "push.v1", metadata: { custom_params: { campaign: "old" } } } }) };
};
...
if (!stage || !stage.textContent.includes("提交后动作") || !stage.textContent.includes("外部推送绑定") || !document.getElementById("opsChannelResourceId")) throw new Error("compiled questionnaire operations page did not retain its existing controls");
...
originalTest.click();
await wait(50);
if (testPosts !== 1 || !originalTest.textContent.includes("测试已创建") || !logCard.textContent.includes("等待发送")) throw new Error("original test button was not taken over by the Host exactly once");
document.querySelector('[data-survey-log-scope="global"]').click();
await wait(10);
if (!logCard.textContent.includes("已收到测试结果") || !logCard.textContent.includes("结果待确认（不会自动重复发送）")) throw new Error("actual log card did not show true effect states");
...
postTab.click();
await wait(10);
if (!document.getElementById("opsChannelResourceId") || !document.querySelector("[data-survey-push-metadata]") || !document.querySelector("[data-survey-host-logs]")) throw new Error("switching the existing tab removed the Host or submission-after-action controls");
dom.window.close();
