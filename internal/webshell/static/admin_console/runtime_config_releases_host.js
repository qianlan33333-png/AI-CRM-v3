(() => {
  ...
  const showDetail = async (id, notice = "") => {
    ...
    try {
      ...
      if (["published", "superseded"].includes(release.state)) { const rollback = button("创建回滚发布", "danger"); rollback.dataset.runtimeReleaseRollback = String(id); rollback.addEventListener("click", () => void postAction("rollback").catch((error) => status(error.message, "error"))); actions.append(rollback); }
