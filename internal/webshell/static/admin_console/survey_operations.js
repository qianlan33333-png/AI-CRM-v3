(function () {
  ...
  function statusLabel(item) {
    if (item.status === 'queued') return '等待发送'; if (item.status === 'executed') return item.provider_result_received === true ? '已收到测试结果' : '已完成处理';
    if (item.status === 'outcome_unknown') return '结果待确认（不会自动重复发送）'; if (item.status === 'disabled') return '当时未启用外推配置';
  ...
  function renderLogs(card, current, global, scope, keyword, setScope) {
    ...
    const header = document.createElement('div'); header.style.cssText = 'display:flex;justify-content:space-between;gap:8px;align-items:center'; const heading = document.createElement('h3'); heading.textContent = scope === 'global' ? '全部问卷测试记录' : '当前问卷测试记录'; heading.style.cssText = 'margin:0;font-size:15px'; const count = document.createElement('span'); count.style.cssText = 'font-size:12px;color:#8F959E'; header.append(heading, count); card.appendChild(header);
  ...
  function installLegacyQuestionnaireOpsGuard() {
    if (document.body.dataset.page !== 'questionnaireOps') return;
    window.fetch = function (input, options) { const method = (options && options.method || (input && input.method) || 'GET').toUpperCase(); const url = typeof input === 'string' ? input : input && input.url || ''; if (method === 'GET' && /^\/admin\/questionnaires\/[1-9][0-9]*\/external-push-logs(?:\?|$)/.test(url)) return Promise.resolve({ok:true,status:200,json:function () { return Promise.resolve({items:[],total:0,limit:100,offset:0,has_more:false,local_only:true}); }}); return platformFetch(input, options); };
    document.addEventListener('click', function (event) { const button = event.target && event.target.closest && event.target.closest('button'); if (button && button.textContent && button.textContent.includes('测试推送（仅本地记录）')) { event.preventDefault(); event.stopImmediatePropagation(); if (typeof window.__surveyHostTestPush === 'function') void window.__surveyHostTestPush(button); } }, true);
  }
  function start() {
