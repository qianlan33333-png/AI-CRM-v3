(() => {
  ...
  const button = (label, kind = "ghost") => {
    const node = element("button", `admin-button admin-button--${kind}`, label);
    node.type = "button";
    return node;
  };
  const csrf = () => {
  ...
  const decode = (raw) => { try { return JSON.parse(raw); } catch (_) { return undefined; } };
  ...
  const categoryState = (category, model) => {
    ...
    if (category.managed_url) return { label: "由业务 Owner 管理", detail: "该类别没有 Config 写入口。" };
    ...
    if (!required.size) return { label: "无可配置运行时字段", detail: "没有可发布的 V3 consumer。" };
    ...
    if (missing.length) return { label: "已发布，待受控应用", detail: `revision #${effective.revision} 尚未由 ${missing.join("、")} 启动读取。` };
    return { label: "已发布并已由所需角色读取", detail: `revision #${effective.revision} 的应用记录齐全。` };
  };
  ...
  const showCenter = async () => {
    clear();
    const intro = card("配置中心", "沿用旧版分类和操作顺序：先保存草稿、校验、再发布。密钥只显示受保护引用；发布不会直接改环境、重启进程或调用 Provider。");
    ...
    const wrap = element("div", "admin-table-wrap"); wrap.append(table); root.append(wrap);
    try {
      ...
      for (const category of model.categories || []) {
        ...
        if (category.managed_url) {
          const link = document.createElement("a"); link.className = "admin-button admin-button--ghost"; link.href = category.managed_url; link.textContent = "打开 Owner 页面"; action.append(link);
  ...
  const inputFor = (field, value) => {
    ...
    row.append(element("small", "admin-muted", field.application === "immediate" ? "发布后由业务请求边界读取；仍需发布记录。" : "发布后需重启相应角色读取此 revision。"));
    return row;
  };

  const showCategory = async () => {
    clear();
    const intro = card("配置详情", "正在读取受控运行时快照。");
    ...
    try {
      ...
      intro.querySelector("p").textContent = category.disabled || category.managed_url ? (category.disabled || "此分类由业务 Owner 管理。") : categoryState(category, model).detail;
      if (category.managed_url) {
        const link = document.createElement("a"); link.href = category.managed_url; link.className = "admin-button admin-button--primary"; link.textContent = "打开 Owner 页面"; intro.append(link); return;
      }
      if (category.disabled) return;
      const effective = new Map((model.effective?.settings || []).map((item) => [item.key, decode(item.value)]));
      const form = element("form", "admin-form-grid admin-form-grid--stacked");
      for (const field of category.fields || []) form.append(inputFor(field, effective.get(field.key)));
      form.append(element("p", "admin-muted", "保存只创建草稿；请在发布详情完成跨字段校验和发布。未知键、未列入闭集的密钥引用与不支持字段都会被拒绝。"));
