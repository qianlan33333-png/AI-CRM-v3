...
func (r *Repository) ListCommissionsByCustomer(ctx context.Context, customerID int64, status distributiondomain.CommissionStatus, cursor string, limit int32) (distributionport.CommissionPage, error) {
	...
	q := `SELECT c.id,c.order_id,a.product_name,c.initial_minor,c.current_payable_minor,c.paid_minor,c.status,c.hold_reason,c.cancel_reason,c.exception_reason,c.paid_confirmed_at,c.due_at,(SELECT MAX(ae.occurred_at) FROM distribution_audit_events ae WHERE ae.aggregate_type='commission' AND ae.aggregate_id=c.id AND ae.event_type='distribution.settlement_paid.v1'),c.created_at,'CNY'
