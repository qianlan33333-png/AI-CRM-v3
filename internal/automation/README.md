...
This package is the bounded PR07 preparation slice for Automation-owned Agent
...
`port/events.go` and `port/media.go` are narrow local seams. Terra should
adapt them to the v3 versioned event/outbox and canonical Media ports at the
composition boundary; this package never imports Media stores or dispatches
...
shared dependencies must not become PR07 routes. Production integration must
mount only a v3-owned adapter through PR10 `internal/webshell/admin_base`;
the donor page must never be deployed as a second `.side` shell or edited.
