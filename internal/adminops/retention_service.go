...
func (s *RetentionService) PreviewRetention(ctx context.Context, id string) (RetentionPreview, error) {
	...
	case "ops_diagnostics":
		query = `SELECT count(*),coalesce(sum((coalesce(pg_column_size(r.id),0)+coalesce(pg_column_size(r.component),0)+coalesce(pg_column_size(r.release_sha),0)+coalesce(pg_column_size(r.code),0)+coalesce(pg_column_size(r.correlation_digest),0)+coalesce(pg_column_size(r.route_template),0)+coalesce(pg_column_size(r.job_ref),0)+coalesce(pg_column_size(r.effect_ref),0)+coalesce(pg_column_size(r.occurred_at),0))),0) FROM adminops_diagnostic_events r WHERE occurred_at<$1`
...
func (s *RetentionService) pruneOwn(ctx context.Context, tx pgx.Tx, id string, cutoff time.Time) (int64, int64, error) {
	...
	case "ops_diagnostics":
		query = `WITH candidates AS(SELECT id,(coalesce(pg_column_size(r.id),0)+coalesce(pg_column_size(r.component),0)+coalesce(pg_column_size(r.release_sha),0)+coalesce(pg_column_size(r.code),0)+coalesce(pg_column_size(r.correlation_digest),0)+coalesce(pg_column_size(r.route_template),0)+coalesce(pg_column_size(r.job_ref),0)+coalesce(pg_column_size(r.effect_ref),0)+coalesce(pg_column_size(r.occurred_at),0))::bigint AS bytes FROM adminops_diagnostic_events r WHERE occurred_at<$1 ORDER BY id LIMIT 1000 FOR UPDATE SKIP LOCKED), changed AS(DELETE FROM adminops_diagnostic_events r USING candidates c WHERE r.id=c.id AND r.occurred_at<$1 RETURNING c.bytes) SELECT count(*),coalesce(sum(bytes),0)::bigint FROM changed`
	case "ops_report_payloads":
