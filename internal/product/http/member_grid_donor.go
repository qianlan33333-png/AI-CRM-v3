...
func decodeDonorGridConfig(raw json.RawMessage) (donorGridConfig, error) {
	...
	for _, item := range c.Sorts {
		if !orderOwnedDonorGridField(item.Field) || sortFields[item.Field] || (item.Direction != "asc" && item.Direction != "desc") {
			return donorGridConfig{}, errors.New("unsupported member-grid sort")
		}
		sortFields[item.Field] = true
	}
	for _, item := range c.Groups {
		if !orderOwnedDonorGridField(item.Field) || groupFields[item.Field] || sortFields[item.Field] || (item.Direction != "asc" && item.Direction != "desc") {
	...
	for index := range c.Filter.Conditions {
		...
		if item.Operator == "between" {
			...
			if values[0].(float64) > values[1].(float64) {
				values[0], values[1] = values[1], values[0]
				c.Filter.Conditions[index].Value = values
			}
		}
	}
	return c, nil
...
func donorGridSchema(editable bool) map[string]any {
	...
	fields := []any{
		...
		field("renewal_count", "续费次数", "number", "number", numberOps, true, true, false),
		field("remark", "备注", "text", "text", textOps, true, true, editable),
