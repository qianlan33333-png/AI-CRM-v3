...
function showOwnedPurchase(){purchaseState='owned';const detail=document.getElementById('detailContent');if(detail)detail.hidden=true;revealCheckout();for(const id of ['paymentDetails','mobilePanel','paymentMethod']){const panel=document.getElementById(id);if(panel)panel.hidden=true}button.disabled=true;button.textContent='已购买';statusBox.textContent='您已购买本课程';updateAmounts();if(readCheckout()?.terminal_status==='paid')void restorePaidCheckout();}
