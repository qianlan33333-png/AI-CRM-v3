...
function readCheckout(){try{const value=JSON.parse(localStorage.getItem(checkoutStorageKey)||'null');return value&&typeof value.key==='string'&&value.key.length>=8?value:null}catch(_){return null}}
function writeCheckout(value){try{localStorage.setItem(checkoutStorageKey,JSON.stringify(value))}catch(_){}}
function clearCheckout(){try{localStorage.removeItem(checkoutStorageKey)}catch(_){}}
function checkoutKey(){const existing=readCheckout();if(existing)return existing;const created={key:crypto.randomUUID(),merchant_order_no:''};writeCheckout(created);return created}
