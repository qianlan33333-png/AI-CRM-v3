...
for(const selector of ['.product','#paymentDetails','#mobilePanel','#paymentMethod','.checkout-footer']){const panel=checkoutContent.querySelector(selector);if(panel)panel.hidden=true}
...
function resetCompletionView(){statusBox.className='status';statusBox.replaceChildren();for(const selector of ['.product','#paymentDetails','#mobilePanel','#paymentMethod','.checkout-footer']){const panel=checkoutContent.querySelector(selector);if(panel)panel.hidden=false}}
...
function showOwnedPurchase(action){purchaseState='owned';const detail=document.getElementById('detailContent');if(detail)detail.hidden=true;revealCheckout();for(const id of ['paymentDetails','mobilePanel','paymentMethod']){const panel=document.getElementById(id);if(panel)panel.hidden=true}button.disabled=true;button.textContent='已购买';statusBox.textContent='您已购买本课程';updateAmounts();if(readCheckout()?.terminal_status==='paid'){void restorePaidCheckout();return}try{showCompletionAction(action)}catch(_){statusBox.textContent='已购买，后续指引暂不可用'} }
