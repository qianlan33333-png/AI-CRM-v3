...
async function poll(orderNo){for(let i=0;i<90;i++){const value=await requestJSON('/api/v1/wechat-pay/checkouts/'+encodeURIComponent(orderNo),{credentials:'same-origin'});if(value.status==='paid'){retainPaidCheckout(orderNo);statusBox.textContent='支付成功';showCompletionAction(value.completion_action);if(value.completion_action&&value.completion_action.state==='unavailable')statusBox.textContent='支付成功，后续指引暂不可用';button.disabled=true;restartButton.hidden=false;return}if(value.checkout_abandoned===true){statusBox.textContent='原支付流程已停止，订单记录已保留，尚未确认最终结果。请联系管理员核对后再购买。';button.disabled=true;restartButton.hidden=true;return}if(value.status==='failed'||value.status==='cancelled'){clearCheckout();throw new Error('支付未完成，请确认后重新购买')}if(value.prepay_state==='outcome_unknown')throw new Error('微信支付下单结果尚未确认，原订单已保留，请稍后查看；请勿重复下单');if(value.prepay_state==='final_failed')throw new Error('微信支付暂不可用，原订单已保留，请联系客服处理');if(value.ready&&value.handoff&&!button.dataset.invoked){button.dataset.invoked='1';try{await invokePay(value.handoff)}catch(error){button.dataset.invoked='';throw error}}await sleep(1500)}button.dataset.invoked='';throw new Error('支付结果确认超时，请稍后刷新查看')}
