...
func (h *PublicHandler) publicPage(w http.ResponseWriter, r *http.Request, payment bool) {
	...
	data := struct {
		...
		Payment bool
		Detail  bool
	}{Product: product, Payment: true, Detail: !payment && len(product.Images) > 0}
...
const button=document.getElementById('buy'),statusBox=document.getElementById('status'),couponField=document.getElementById('coupon'),identityGate=document.getElementById('identityGate'),identityMessage=document.getElementById('identityMessage'),authContinue=document.getElementById('authContinue'),checkoutContent=document.getElementById('checkoutContent'),inWechat=/MicroMessenger/i.test(navigator.userAgent),checkoutStorageKey='aicrm.checkout.tab.v2:'+{{.Product.ID}}+':{{.Product.ProductKind}}';authContinue.href='/api/h5/wechat-pay/oauth/start?return_url='+encodeURIComponent(location.pathname)
let purchaseState='available';const authAttemptKey='aicrm.oauth.auto:'+location.pathname;function authorizeMissingSession(message){showIdentityGate(message);if(!inWechat)return;try{if(sessionStorage.getItem(authAttemptKey))return;sessionStorage.setItem(authAttemptKey,'1')}catch(_){return}authContinue.hidden=true;identityMessage.textContent='正在前往微信授权…';location.assign(authContinue.href)}authContinue.addEventListener('click',()=>{try{sessionStorage.removeItem(authAttemptKey)}catch(_){}});function showIdentityGate(message){const detail=document.getElementById('detailContent');if(detail)detail.hidden=true;checkoutContent.hidden=true;identityGate.hidden=false;identityMessage.textContent=message;if(inWechat){authContinue.hidden=false;authContinue.removeAttribute('aria-disabled')}else{authContinue.hidden=false;authContinue.textContent='请在微信中打开';authContinue.setAttribute('aria-disabled','true')}}function revealCheckout(){try{sessionStorage.removeItem(authAttemptKey)}catch(_){}identityGate.hidden=true;checkoutContent.hidden=false}function revealProduct(){const detail=document.getElementById('detailContent');if(!detail)return;checkoutContent.hidden=true;detail.hidden=false;for(const image of detail.querySelectorAll('img[data-src]'))image.src=image.dataset.src;document.getElementById('detailPrice').textContent=({{.Product.PriceMinor}}/100).toFixed(2)}
...
async function poll(orderNo){for(let i=0;i<90;i++){const value=await requestJSON('/api/v1/wechat-pay/checkouts/'+encodeURIComponent(orderNo),{credentials:'same-origin'});readOriginalAmount(value,orderNo);if(value.status==='paid'){retainPaidCheckout(orderNo);statusBox.textContent='支付成功';showCompletionAction(value.completion_action);if(value.completion_action&&value.completion_action.state==='unavailable')statusBox.textContent='支付成功，后续指引暂不可用';button.disabled=true;if(renewButton)renewButton.hidden=false;button.textContent='已购买';return}if(releaseExpiredCheckout(value,orderNo))return;if(value.checkout_abandoned===true){statusBox.textContent='原支付流程已停止，订单记录已保留，尚未确认最终结果。请联系管理员核对后再购买。';button.disabled=true;if(renewButton)renewButton.hidden=true;return}if(value.status==='failed'||value.status==='cancelled'){clearCheckout();originalAmount=null;updateAmounts();throw new Error('支付未完成，请确认后重新购买')}if(value.prepay_state==='outcome_unknown')throw new Error('微信支付下单结果尚未确认，原订单已保留，请稍后查看；请勿重复下单');if(value.prepay_state==='final_failed')throw new Error('微信支付暂不可用，原订单已保留，请联系客服处理');if(value.ready&&value.handoff&&!button.dataset.invoked){button.dataset.invoked='1';try{await invokePay(value.handoff)}catch(error){button.dataset.invoked='';throw error}}await sleep(1500)}button.dataset.invoked='';throw new Error('支付结果确认超时，请稍后刷新查看')}
