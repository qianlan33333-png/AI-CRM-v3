...
function checkoutPayload(value){if(!value||typeof value!=='object'||value.product_id!=={{.Product.ID}}||value.product_kind!=='{{.Product.ProductKind}}'||value.beneficiary_selection!=='payer_self'||!Number.isSafeInteger(value.coupon_claim_id)||value.coupon_claim_id<0||(value.promotion_context||'')!==promotionContext)return null;if(value.mobile!==undefined&&(typeof value.mobile!=='string'||!/^\+861[3-9][0-9]{9}$/.test(value.mobile)))return null;const normalized={product_id:{{.Product.ID}},product_kind:'{{.Product.ProductKind}}',beneficiary_selection:'payer_self',coupon_claim_id:value.coupon_claim_id};if(promotionContext)normalized.promotion_context=promotionContext;if(value.mobile!==undefined)normalized.mobile=value.mobile;return normalized}
