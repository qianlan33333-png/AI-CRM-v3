...
function readCheckout(){try{const value=JSON.parse(localStorage.getItem(checkoutStorageKey)||'null'),payload=checkoutPayload(value&&value.payload);return value&&typeof value.key==='string'&&value.key.length>=8&&typeof value.merchant_order_no==='string'&&payload?(value.payload=payload,value):null}catch(_){return null}}
...
function checkoutKey(payload){const existing=readCheckout();if(existing)return existing;const created={key:crypto.randomUUID(),merchant_order_no:'',payload:checkoutPayload(payload)};return created.payload&&writeCheckout(created)?created:null}
const sleep=ms=>new Promise(resolve=>setTimeout(resolve,ms));function requestFailure(code,message){const error=new Error(message);error.code=code;return error}async function requestJSON(url,options){const response=await fetch(url,options);let body={};try{body=await response.json()}catch(_){}if(response.status===401){location.href='/api/h5/wechat-pay/oauth/start?return_url='+encodeURIComponent(location.pathname);throw requestFailure('payment_session_required','正在进入微信授权')}if(!response.ok){const code=body.code||body.error||'';if(code==='payment_provider_disabled'||code==='payment_h5_oauth_disabled')throw requestFailure(code,'支付服务暂未启用');if(code==='conflict')throw requestFailure(code,'商品或手机号状态不符合购买要求');throw requestFailure(code,'请求失败')}return body}
