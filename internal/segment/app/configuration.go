...
// It is primarily used by semantic bootstrap so a stale idempotency receipt
// cannot hide a package that was already created by an earlier release.
...
// PackageCodeForIdempotencyKey is the stable code derivation used by package
// creation. Bootstrap uses it to reconcile durable rows before replaying an
// operation receipt from an older bootstrap generation.
