...
const packageColumns = `id,group_id,code,name,lifecycle,version,current_configuration_version_id,published_snapshot_id,current_automation_binding_id,current_sender_set_id,created_by,updated_by,created_at,updated_at,archived_at`

func scanPackage(row pgx.Row) (segmentdomain.Package, error) {
	var item segmentdomain.Package
	var lifecycle string
	err := row.Scan(&item.ID, &item.GroupID, &item.Code, &item.Name, &lifecycle, &item.Version, &item.CurrentConfigurationVersionID, &item.PublishedSnapshotID, &item.CurrentAutomationBindingID, &item.CurrentSenderSetID, &item.CreatedBy, &item.UpdatedBy, &item.CreatedAt, &item.UpdatedAt, &item.ArchivedAt)
	if errors.Is(err, pgx.ErrNoRows) {
