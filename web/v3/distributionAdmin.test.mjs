...
const dom = new JSDOM('<!doctype html><main id="distribution-admin-root"></main>', { url: 'https://crm.example/admin/distribution', runScripts: 'outside-only', pretendToBeVisual: true, beforeParse(window) {
  ...
  window.fetch = async (input, init = {}) => { const url = new URL(String(input), window.location.href); calls.push({ path: url.pathname, search: url.search, method: init.method || 'GET', body: init.body || '', idempotencyKey: new Headers(init.headers).get('Idempotency-Key') || '' });
    ...
    if (url.pathname === '/api/admin/distribution/orders') return json({ items: [{ attribution_id: 'a1', order_reference: 'O-9', item_line: '1', product_id: 7, product_type: 'standard_product', product_name: '增长课', distributor_display_name: '<img src=x onerror=window.__nameXss=1>', qualification_state: 'eligible', qualification_evidence_reference: 'evidence:1', policy_version: 2, rate_basis_points: 333, wait_days: 7, paid_minor: 19900, currency: 'CNY', attributed_at: '2026-09-14T00:00:00Z' }], next_cursor: '' });
    if (url.pathname === '/api/admin/distribution/exceptions') return json({ items: [{ exception_id: 'x1', commission_id: 'c1', distributor_display_name: '<img src=x onerror=window.__nameXss=1>', order_reference: 'O-9', kind: 'qualification_revoked', status: 'open', unpaid_due_minor: 12, already_paid_minor: 33, amount_minor: 12, reason: '资格退款', payment_instruction_reference: 'pi:1', created_at: '2026-09-14T00:00:00Z', updated_at: '2026-09-14T00:00:00Z', version: 4, can_reconcile: true, can_record_recovery: true, can_record_merchant_liability: true }], next_cursor: '' });
