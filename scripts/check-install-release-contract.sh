...
grep -qF 'node scripts/run-with-donor-views.mjs -- bash scripts/run-donor-view-consumers.sh release' "$ci_workflow" || { echo "CI must invoke the reviewed release builder through disposable donor views" >&2; exit 1; }
...
grep -qF './cmd/migrate-open-platform' .github/workflows/ci.yml || { echo "CI must test the Open Platform history tool" >&2; exit 1; }
