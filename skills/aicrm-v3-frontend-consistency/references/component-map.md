...
| 公共问卷 H5 | `web/v3/surveyPublicHost.ts`、`web/v3/surveyPublic.css` | `internal/survey/http.Handler.publicEntry` 的 `/q/{slug}` 保留 OAuth/session 入口；构建阶段只为 `auth`、`all`、`one`、`result`、`error` 注入 Host 与样式，再由 Survey manifest 装配 | Host 只呈现既有题目、校验、提交中、错误、失效链接与结果回读状态；提交 key、答案写入、OAuth/OneID 和结果 token 始终归 Survey Owner。 |
