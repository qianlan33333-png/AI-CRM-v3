...
try {
 ...
 await value(cdp,"(()=>{const details=document.querySelector('#material-refresh-panel [data-material-refresh-details]');if(!(details instanceof HTMLDetailsElement))return false;details.open=true;details.dispatchEvent(new Event('toggle'));return true})()");
 await wait(cdp,`Boolean(document.querySelector('#material-refresh-panel')?.textContent.includes(${JSON.stringify(missingSourceRef)})&&document.querySelector('#material-refresh-panel')?.textContent.includes('原文件缺失，请补传'))`,`expanded refresh details and missing source ${missingSourceRef}`);
