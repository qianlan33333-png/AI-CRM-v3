...
try {
  ...
  try {
    await waitFor(cdp, "document.querySelector('[data-external-push-configuration]')?.textContent.includes('配置版本 2')", "browser configuration save did not finish");
  } catch (_) {
  ...
  try {
    await waitFor(cdp, "document.querySelector('[data-external-push-configuration]')?.textContent.includes('配置版本 2')", "service-period browser configuration save did not finish");
  } catch (_) {
