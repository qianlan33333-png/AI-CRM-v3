...
try {
  ...
  if (phase === 'registration') {
    ...
    await wait(cdp,"[...document.querySelectorAll('#distribution-root button')].some(x=>x.textContent==='获取分销链接')",'Payment worker completion did not project ready receiver into the public UI');
  ...
  await value(cdp, "[...document.querySelectorAll('button')].find(x=>x.textContent==='获取分销链接')?.click(); true");
  ...
  await login(cdp); await cdp.call("Page.navigate",{url:`${base}/admin/distribution`}); await sleep(200); await value(cdp,"import(document.querySelector('script')?.src).then(()=>true)"); await wait(cdp,`document.querySelector('#distribution-admin-root')?.textContent.includes(${JSON.stringify(adminDisplayName)})`,'Distribution admin distributor view did not read the Customer directory nickname');
