...
if (!/^https:\/\//.test(base || "") || !distributorSession || !product || !/^[1-9][0-9]*$/.test(productID || "") || !csrf || !/^[1-9][0-9]*$/.test(detailAttribution || "") || !/^[1-9][0-9]*$/.test(detailException || "") || !detailCreatedAt || !admin || !password) throw new Error("Distribution Chromium journey environment is incomplete");
...
try {
  ...
  await cdp.call("Page.navigate",{url:`${base}/distribution?product_id=${productID}&product_type=standard_product`}); await sleep(200); await value(cdp,"import(document.querySelector('script')?.src).then(()=>true)"); await wait(cdp,"document.querySelector('#distribution-root')?.textContent.includes('申请成为分销员')",`Distribution registration did not render ${await value(cdp,"(async()=>JSON.stringify({path:location.pathname,text:document.querySelector('#distribution-root')?.textContent||'',me:await fetch('/api/v1/distribution/me').then(async r=>[r.status,await r.text()])}))()")}`);
  ...
  await wait(cdp,"document.querySelector('#distribution-root')?.textContent.includes('微信收款准备未完成')",'first trusted session did not create its distributor registration');
  await value(cdp,"[...document.querySelectorAll('#distribution-root button')].find(x=>x.textContent==='完成收款准备')?.click(); true");
  await wait(cdp,"document.querySelector('#distribution-root')?.textContent.includes('生成推广入口')",'registered distributor did not read its existing ready receiver');
  await value(cdp, "[...document.querySelectorAll('button')].find(x=>x.textContent==='生成推广入口')?.click(); true");
  ...
  if (!(await value(cdp, credentialDialog))) {
    ...
    throw new Error(`qualified distributor did not receive real credential page=${page} assets=${JSON.stringify(cdp.responses.filter(x => x.url.includes('/assets/')).slice(-8))}`);
  }
  const credentialURL=await value(cdp, "document.querySelector('dialog input[readonly]')?.value"); assert.match(credentialURL, new RegExp(`^${base.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')}/d/dpc_[A-Za-z0-9_-]{16,}$`)); const generatedPromotion=new URL(credentialURL).pathname.slice(3);
  await wait(cdp, "Boolean(document.querySelector('dialog .distribution-qr svg'))", 'credential QR did not render');
  await value(cdp, "document.querySelector('dialog button')?.click(); true");
  ...
  if (cdp.responses.filter(item => item.url.includes('promotion-credentials')).length <= credentialResponseCount) throw new Error('credential replay did not issue a real HTTP request');
  ...
  await wait(cdp, credentialDialog, 'credential replay did not show its original link'); assert.equal(await value(cdp, "document.querySelector('dialog input[readonly]')?.value"),credentialURL,'same idempotency key must return the original URL');
  await value(cdp, "document.querySelector('dialog button')?.click(); true");
  await value(cdp,"[...document.querySelectorAll('button')].find(x=>x.textContent==='我的收益')?.click(); true"); await wait(cdp,"document.querySelector('#distribution-root')?.textContent.includes('累计推广成交额')","new distributor earnings did not render its summary");
  ...
  await login(cdp); await cdp.call("Page.navigate",{url:`${base}/admin/distribution`}); await sleep(200); await value(cdp,"import(document.querySelector('script')?.src).then(()=>true)"); await wait(cdp,"document.querySelector('#distribution-admin-root')?.textContent.includes('DISTBROWSER01')","Distribution admin distributor view did not read back the real distributor");
