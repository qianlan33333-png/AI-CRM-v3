...
try {
  ...
  for (const detailCase of [
    ...
    orderDetailEvidence.push({provider:detailCase.provider,product:detailCase.product,settlement:detailCase.settlement,confirmed:detailCase.confirmed,url:detailCase.row.detailURL});
  }
  if(screenshotDir)await fs.writeFile(path.join(screenshotDir,'order-browser-evidence.json'),JSON.stringify({revision,order_collision_reference:orderCollisionReference,settlement_confirmed_at:orderSettledAt,list_rows:orderRows.map(row=>({detail_url:row.detailURL})),details:orderDetailEvidence,screenshots:['order-list-1280.png','order-list-1440.png','order-detail-1280.png','order-detail-1440.png']},null,2));
