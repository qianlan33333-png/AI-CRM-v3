...
try {
  ...
  if (!shellLayout?.stage || !shellLayout?.topbar || shellLayout.headers !== 1 || shellLayout.titleText !== "群运营计划" || shellLayout.pageH1Count !== 1 || !shellLayout.root || !shellLayout.detail || shellLayout.syntheticWorkspaceHeadings !== 0 || shellLayout.stage.paddingLeft !== "28px" || shellLayout.stage.paddingTop !== "20px" || shellLayout.root.top + 1 < shellLayout.topbar.bottom || Math.abs(shellLayout.root.left - shellLayout.stage.left - 28) > 1 || Math.abs(shellLayout.root.top - shellLayout.stage.top - 20) > 1 || Math.abs(shellLayout.detail.left - shellLayout.root.left) > 1 || Math.abs(shellLayout.detail.top - shellLayout.root.top) > 1) throw new Error("Group Ops native shell title/content geometry is invalid");
