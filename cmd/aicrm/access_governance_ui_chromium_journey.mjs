...
try {
  ...
  await evaluate(cdp, "(() => { const input=document.querySelector('#admin-access-employee-search'); input.value='UnavailableID'; input.dispatchEvent(new Event('input', {bubbles:true})); })()");
  await waitFor(cdp, "document.querySelector('#admin-access-employee-search-status')?.textContent.includes('暂时不可用') && document.querySelector('#admin-access-employee-results')?.textContent.trim() === ''", "directory 503 state was rendered as an empty employee result");
  await evaluate(cdp, "(() => { const input=document.querySelector('#admin-access-employee-search'); input.value='CandidateCaseID'; input.dispatchEvent(new Event('input', {bubbles:true})); })()");
  ...
  await evaluate(cdp, "document.querySelector('#admin-access-provision').click(); true");
  await evaluate(cdp, "(() => { const input=document.querySelector('#admin-access-employee-search'); input.value='CandidateCaseID'; input.dispatchEvent(new Event('input', {bubbles:true})); input.value='SecondCandidateID'; input.dispatchEvent(new Event('input', {bubbles:true})); })()");
