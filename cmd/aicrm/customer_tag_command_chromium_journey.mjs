...
try {
  ...
  try { await waitFor(cdp, "(()=>{const add=document.querySelector('#customer-tag-batch [name=\"add_tag_ids\"]');const entry=add?.parentElement?.querySelector('button[type=button]');return document.querySelectorAll('#customer-list-body input[type=checkbox]').length>=2&&document.querySelectorAll('#customer-tag-batch option').length>=2&&entry instanceof HTMLButtonElement&&!entry.disabled})()", "customer list, tag catalog, or real tag selection entry did not load"); } catch (_) { throw new Error(`customer Host did not load: ${await diagnostic()}`); }
  const openedAddPicker = await evaluate(cdp, "(()=>{const add=document.querySelector('#customer-tag-batch [name=\"add_tag_ids\"]');const entry=add?.parentElement?.querySelector('button[type=button]');if(!(entry instanceof HTMLButtonElement))return false;entry.click();return true})()");
  if (!openedAddPicker) throw new Error(`customer tag add entry did not trigger the standard picker: ${await diagnostic()}`);
  try { await waitFor(cdp, "Boolean(document.querySelector('.aicrm-tag-picker:not([hidden]) [data-tag-key]'))", "customer tag add picker did not open"); } catch (_) { throw new Error(`customer tag add picker did not open: ${await diagnostic()}`); }
  ...
  // Read the served stable Host and require the browser's real module requests
  // for every rebased V3 chunk. This catches a copied stable entry resolving
  // ./chunks beneath standard-components even when the picker source itself
  ...
  const confirmedAdd = await evaluate(cdp, "(()=>{const picker=document.querySelector('.aicrm-tag-picker:not([hidden])');const tag=picker?.querySelector('[data-tag-key]');const confirm=picker?.querySelector('[data-action=\"confirm\"]');if(!(tag instanceof HTMLButtonElement)||!(confirm instanceof HTMLButtonElement))return false;tag.click();picker.querySelector('[data-action=\"confirm\"]')?.click();return true})()");
  if (!confirmedAdd) throw new Error('customer tag add picker did not expose a selectable tag and confirm action');
  await waitFor(cdp, "document.querySelectorAll('#customer-tag-batch [name=\"add_tag_ids\"] option:checked').length===1", "customer tag add selection did not return to the real form");
  const openedRemovePicker = await evaluate(cdp, "(()=>{const remove=document.querySelector('#customer-tag-batch [name=\"remove_tag_ids\"]');const entry=remove?.parentElement?.querySelector('button[type=button]');if(!(entry instanceof HTMLButtonElement))return false;entry.click();return true})()");
  if (!openedRemovePicker) throw new Error('customer tag remove entry was unavailable');
  await waitFor(cdp, "Boolean(document.querySelector('.aicrm-tag-picker:not([hidden]) [data-tag-key]'))", "customer tag remove picker did not open");
  const confirmedRemove = await evaluate(cdp, "(()=>{const picker=document.querySelector('.aicrm-tag-picker:not([hidden])');const tags=[...picker.querySelectorAll('[data-tag-key]')];if(tags.length<2)return false;tags.at(-1).click();picker.querySelector('[data-action=\"confirm\"]')?.click();return true})()");
  if (!confirmedRemove) throw new Error('customer tag remove picker did not expose the second selectable tag and confirm action');
  await waitFor(cdp, "document.querySelectorAll('#customer-tag-batch [name=\"remove_tag_ids\"] option:checked').length===1", "customer tag remove selection did not return to the real form");
  await evaluate(cdp, `(() => { const checks=[...document.querySelectorAll('#customer-list-body input[type=checkbox]')].slice(0,2); checks.forEach((box) => { box.checked=true; box.dispatchEvent(new Event('change', {bubbles:true})); }); const form=document.querySelector('#customer-tag-batch'); window.confirm=()=>true; form.requestSubmit(); return true; })()`);
