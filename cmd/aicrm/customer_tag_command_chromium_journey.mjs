...
try {
  ...
  try { await waitFor(cdp, "document.querySelectorAll('#customer-list-body input[type=checkbox]').length >= 2 && document.querySelectorAll('#customer-tag-batch option').length >= 2 && typeof window.AICRMWeComTagPicker?.open === 'function'", "customer list, tag catalog, or standard picker did not load"); } catch (_) { throw new Error(`customer Host did not load: ${await diagnostic()}`); }
