...
try {
  ...
  await resize(360);
  const composerNarrowPreview = await evaluate(cdp, "(() => { const dialog=document.querySelector('[data-v3-content-composer] .aicrm-content-composer'); const body=dialog?.querySelector('.aicrm-content-composer__body'); const preview=dialog?.querySelector('[data-v3-composer-preview]'); const footer=dialog?.querySelector('footer'); if (!(body instanceof HTMLElement) || !(preview instanceof HTMLElement) || !(footer instanceof HTMLElement)) return {ready:false}; body.scrollTop=body.scrollHeight; const bodyBox=body.getBoundingClientRect(); const previewBox=preview.getBoundingClientRect(); const confirm=footer.querySelector('[data-v3-composer-confirm]')?.getBoundingClientRect(); return {ready:true,scrollable:body.scrollHeight>body.clientHeight+1,scrolled:body.scrollTop>0,previewFits:previewBox.height<=bodyBox.height+1,previewVisible:previewBox.top>=bodyBox.top-1&&previewBox.bottom<=bodyBox.bottom+1,confirmVisible:Boolean(confirm&&confirm.top>=0&&confirm.bottom<=innerHeight),overflow:document.documentElement.scrollWidth>innerWidth+1}; })()");
  if (!composerNarrowPreview.ready || !composerNarrowPreview.scrollable || !composerNarrowPreview.scrolled || !composerNarrowPreview.previewFits || !composerNarrowPreview.previewVisible || !composerNarrowPreview.confirmVisible || composerNarrowPreview.overflow) throw new Error("narrow content composer could not scroll to its complete preview and fixed confirmation: " + JSON.stringify(composerNarrowPreview));
