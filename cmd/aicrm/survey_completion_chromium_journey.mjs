...
try {
  ...
  await waitFor(cdp, "(() => { const heading=[...document.querySelectorAll('h3')].find((item) => item.textContent.trim()==='外部推送绑定'); return Boolean(heading && heading.parentElement?.parentElement && [...heading.parentElement.parentElement.querySelectorAll('span')].some((item) => item.getAttribute('style')?.includes('cursor'))); })()", 'external-push controls did not render');
