...
func (i *importer) groups() error {
	...
	for index, row := range rows {
		...
		disposition := "imported"
		if errors.Is(e, pgx.ErrNoRows) {
			e = i.tx.QueryRow(i.ctx, `INSERT INTO segment_audience_groups(name,sort_order,version,created_by,created_actor_kind,created_actor_ref,updated_by,updated_actor_kind,updated_actor_ref,created_at,updated_at) VALUES($1,$2,1,$3,'admin',$4,$3,'admin',$4,$5,$6) RETURNING id`, row.Name, row.SortOrder, i.actor, i.actorRef, row.CreatedAt, row.UpdatedAt).Scan(&targetID)
		} else if e == nil {
			disposition = "mapped"
		}
		if e != nil {
...
func (i *importer) packagesAndConfigurations() error {
	...
	for index, row := range packages {
		...
		if errors.Is(e, pgx.ErrNoRows) {
			...
			e = i.tx.QueryRow(i.ctx, `INSERT INTO segment_audience_packages(group_id,code,name,lifecycle,version,created_by,created_actor_kind,created_actor_ref,updated_by,updated_actor_kind,updated_actor_ref,created_at,updated_at,archived_at) VALUES($1,$2,$3,$4,1,$5,'admin',$6,$5,'admin',$6,$7::timestamptz,$8::timestamptz,CASE WHEN $4='archived' THEN $8::timestamptz ELSE NULL::timestamptz END) RETURNING id`, groupID, code, row.Name, map[bool]string{true: "archived", false: "paused"}[archived], i.actor, i.actorRef, row.CreatedAt, row.UpdatedAt).Scan(&targetID)
		} else if e == nil {
			disposition = "mapped"
		}
		if e != nil {
		...
		for _, configIndex := range indices {
			...
			if errors.Is(e, pgx.ErrNoRows) {
				...
				if e = i.tx.QueryRow(i.ctx, `SELECT COALESCE(max(version),0)+1 FROM segment_audience_configuration_versions WHERE package_id=$1`, targetID).Scan(&next); e == nil {
					e = i.tx.QueryRow(i.ctx, `INSERT INTO segment_audience_configuration_versions(package_id,version,schema_version,definition,refresh_cron_utc,refresh_mode,digest,created_by,created_actor_kind,created_actor_ref,created_at) VALUES($1,$2,1,$3::jsonb,NULLIF($4,''),$5,$6,$7,'admin',$8,$9::timestamptz) RETURNING id`, targetID, next, definition, refresh.Cron, refresh.Mode, definitionDigest[:], i.actor, i.actorRef, c.CreatedAt).Scan(&existingID)
				}
			} else if e == nil {
				cdisp = "duplicate"
			}
...
func (i *importer) senders() error {
	...
	for sourcePackage, indices := range byPackage {
		...
		if err = i.tx.QueryRow(i.ctx, `INSERT INTO segment_audience_sender_sets(package_id,version,created_by,created_actor_kind,created_actor_ref,created_at) VALUES($1,$2,$3,'admin',$4,$5) RETURNING id`, packageID, next, i.actor, i.actorRef, i.snapshot.Manifest.SnapshotAt).Scan(&setID); err != nil {
			return err
		}
