...
| Audience 管理（归档人群包、删除空群组、归档策略、解绑自动化） | `AICRMConfirmationDialog` | `admin_audience_detail.js` 在请求前冻结既有目标与可见名称；组件只持有临时原因、校验、焦点、Escape、取消/确认结果与 busy 状态。既有 Audience Owner 命令仍由 caller 以原路径、正文、鉴权和幂等键发出。 | 浏览器覆盖取消零写入、确认单次归档与 Owner 读回；失败反馈后可按既有命令合同重试。 |
