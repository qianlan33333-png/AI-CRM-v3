...
- `admin.ts`、`controller.ts`、`legacy.ts`、`health.schemas.ts`、`shared/api/*` 是原样共享依赖，内部保留客户/受众/Campaign 等旧分支。它们只能作为 archive source evidence，不能整体接入 v3；Web 只提取 Group Ops mount 和其本领域 DTO。
