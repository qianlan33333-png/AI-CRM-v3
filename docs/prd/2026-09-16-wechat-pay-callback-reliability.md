...
```text
OneID: not involved — 回调和对账只迁移既有 Payment/Order，不解析、创建或关联客户身份。
...
External Effects: no new Provider write — 只复用既有 payment reconciliation jobqueue 和签名 Provider read，不创建新的支付、退款或消息外部效果。
```
