...
3. `legacy_custom` 对可精确解释的固定日 cron 显示实际上海时间，例如 `0 1 * * *` 显示“每日 09:00（上海时间；历史自定义计划）”。不能把它称作“每日 2:00”。无法无歧义转成固定每日时刻的 cron 仍明确标为历史自定义规则，不伪造时间。
4. 新建策略的时区固定 `Asia/Shanghai`，安静时段按上海墙上时间提交。已存策略、历史版本和已排队效果保持原值。
...
- 保存 legacy custom 的其他字段不改变原 `refresh_mode` 或 cron；`0 1 * * *` 在页面显示上海 09:00。
