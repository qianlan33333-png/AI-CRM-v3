...
8. 同一非历史 WeChat Pay Payment 在 Payment 行锁和同一 UoW 中，原键重放之后检查既有退款：存在 `requested`、`effect_accepted` 或 `outcome_unknown` 时拒绝不同 key 的新申请；只有该笔退款已终态后，才允许以新 key 发起后续部分退款。WeChat Shop 的 SKU 退款合同本项不改变。
