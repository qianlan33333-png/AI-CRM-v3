...
> 供体基线：`AI-CRM-production@4af15e64fb7ebb311b52b17eaf5fc5ea5e8154c8`、`AI-CRM-v2@6bfbe5816bb89913c70adaca87d6a486260e016e`

## 1. 执行结论

AI-CRM v3 是后续唯一的新能力主线。它不是 `AI-CRM-production` 的删减分支，也不是 `AI-CRM-v2` 的复制仓，而是一个从空仓建立、按白名单吸收资产的全新主仓库。
...
| `AI-CRM-v2` | Go 领域实现供体、测试和契约参考仓 | 线上止血、迁移适配、读取型对照 | 新产品能力、继续扩张 legacy 兼容层 |
...
- `AI-CRM-v2` 已形成 Go 模块化单体的领域边界、Identity typed contract、Unit of Work、事件、企微客户端、微信支付 Provider、OpenAPI 和较完整的 CRM 业务能力。
...
- 代码树中不存在对 `AI-CRM-production` 或 `AI-CRM-v2` 包的运行时 import。
